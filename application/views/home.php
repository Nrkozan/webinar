        ...
        <div class="content-wrapper">
            ...
            <div class="panel panel-flat">
                ...
                <table class="table datatable-basic table-striped table-xxs">
                    ...
                    <tbody>
                    <?php $user = $this->main_model->getRow('user',['id' => $this->session->userdata('id')])?>

                    <?php foreach ($this->main_model->getTable('server',['active' => 1]) as $main): ?>
                        <?php foreach ($this->main_model->getTable('records',['server' => $main->id]) as $row): ?>
                            <?php if ($user->recordTag): ?>
                                <?php if(strpos($row->name,$user->recordTag) === 0): ?>
                                    <tr>
                                        <td>
                                            <?php echo $row->name?>
                                        </td>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <?php echo $row->playback->format->length?> .dk
                                    </td>
                                    <td>
                                        <?php echo $main->name?>
                                    </td>
                                    <td>
                                        <a target="_blank" href="<?php echo $row->playback->format->url?>">İzle</a>
                                    </td>
                                    <td>
                                        <?php $link = $main->base_url . 'download/presentation/'.$row->recordID.'/'.$row->recordID.'.mp4' ?>
                                        <a target="_blank" href="<?php echo $link?>">İndir</a>
                                    </td>
                                    <?php if (false): ?>
                                        <td>
                                            <button type="button" class="gdriveadd" data-id="<?php echo $row->id?>" >Ekle</button>
